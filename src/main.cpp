...
// Copyright (c) 2013 CryptocauseCoin developers
...
const string strMessageMagic = "CryptocauseCoin Signed Message:\n";
...
static const int64 nTargetSpacing = 1 * 24 * 60 * 60; // 1 day
static const int64 nInterval = nTargetTimespan / nTargetSpacing; // 1 day
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CryptocauseCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CryptocauseCoinMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("CryptocauseCoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running CryptocauseCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CryptocauseCoinMiner terminated\n");
        throw;
    }
}
